    ...
    <div class="gallery-container">
      <Carousel id="gallery" :items-to-show="1" :wrap-around="false" v-model="currentSlide">
        <Slide v-for="(image, index) in catImages" :key="index">
          <img :src="image" :alt="`Photo of Girl the cat ${index + 1}`" loading="lazy" class="gallery-image" />
        </Slide>
      </Carousel>
    </div>
      ...
      <Carousel id="thumbnails" :items-to-show="5" :wrap-around="false" v-model="currentSlide" ref="carousel">
        <Slide v-for="(image, index) in catImages" :key="index">
          <div class="thumbnail-item" @click="slideTo(index)">
            <img :src="image" :alt="`Photo of Girl the cat ${index + 1}`" loading="lazy" class="thumbnail-image" />
          </div>
        </Slide>
      </Carousel>
    </div>
